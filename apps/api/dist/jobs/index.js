...
export async function startJobs(ctx) {
    ...
    await boss.work(QUEUES.sessionPurge, async () => {
        ...
        if (purged > 0)
            console.log(`[jobs] purged ${String(purged)} expired sessions`);
    });
    /**
    ...
    await boss.work(QUEUES.presenceSweep, async () => {
        const closed = await query(`UPDATE vantara_reading_sessions
          SET ended_at = last_beat_at
        WHERE ended_at IS NULL
          AND last_beat_at < now() - interval '5 minutes'
        RETURNING id`);
        await query(`UPDATE vantara_presence SET status = 'OFFLINE', series_ref = NULL, series_title = NULL,
    ...
    await boss.work(QUEUES.sourceSync, async () => {
        ...
        for (const source of sources) {
            await query(`INSERT INTO vantara_source_verdicts (source_id, source_name, lang)
              VALUES ($1, $2, $3)
         ON CONFLICT (source_id)
         DO UPDATE SET source_name = EXCLUDED.source_name, lang = EXCLUDED.lang`, [source.id, source.name, source.lang]);
        }
