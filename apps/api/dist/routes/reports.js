...
export async function reportRoutes(app, ctx) {
    /**
     * إنشاء بلاغ.
     *
    ...
    app.post('/v1/reports', {
        ...
        const collected = {
            ...
            server: {
                ...
                sourceVerdict: input.sourceId
                    ? (await queryOne(`SELECT verdict FROM vantara_source_verdicts WHERE source_id = $1`, [input.sourceId]))?.verdict ?? null
                    : null,
            },
        };
        ...
        const row = await queryOne(`INSERT INTO vantara_reports
    ...
    app.get('/v1/reports', { preHandler: requireSession(ctx) }, async (request, reply) => {
        ...
        const rows = await query(`SELECT r.id, u.username AS reporter, r.kind, r.state,
        ...
        LIMIT 200`, [session.userId, user.role === 'admin']);
    ...
    app.get('/v1/reports/:id', { preHandler: requireSession(ctx) }, async (request, reply) => {
        ...
        const session = sessionOf(request);
        const user = await ctx.uchiyomi.me(session.token);
        const report = await queryOne(`SELECT r.*, u.username AS reporter
         FROM vantara_reports r
         JOIN vantara_users u ON u.uchiyomi_user_id = r.reporter_id
        WHERE r.id = $1 AND ($2 = true OR r.reporter_id = $3)`, [id, user.role === 'admin', session.userId]);
        if (!report)
            return reply.code(404).send({ error: 'not_found' });
        const actions = await query(`SELECT action, outcome, detail, created_at AS "createdAt"
         FROM vantara_report_actions WHERE report_id = $1 ORDER BY created_at`, [id]);
    ...
    app.post('/v1/reports/:id/actions', { preHandler: requireSession(ctx) }, async (request, reply) => {
        ...
        const exists = await queryOne(`SELECT id FROM vantara_reports WHERE id = $1`, [id]);
        if (!exists)
            return reply.code(404).send({ error: 'not_found' });
        await query(`INSERT INTO vantara_report_actions (report_id, action, outcome, detail)
       VALUES ($1, $2, $3, $4)`, [id, parsed.data.action, parsed.data.outcome, JSON.stringify(scrubDiagnostics(parsed.data.detail ?? {}))]);
        if (parsed.data.outcome === 'success') {
            await query(`UPDATE vantara_reports SET state = 'RESOLVED', resolved_at = now() WHERE id = $1`, [id]);
        }
    ...
    app.post('/v1/deleted-works', { preHandler: requireSession(ctx) }, async (request, reply) => {
        ...
        const snapshot = {
            comments: (await queryOne(`SELECT count(*)::text AS n FROM vantara_comments
            WHERE series_ref = $1 AND deleted_at IS NULL`, [input.seriesRef]))?.n,
            recommendations: (await queryOne(`SELECT count(*)::text AS n FROM vantara_recommendations WHERE series_ref = $1`, [input.seriesRef]))?.n,
            sessions: (await queryOne(`SELECT count(*)::text AS n FROM vantara_reading_sessions WHERE series_ref = $1`, [input.seriesRef]))?.n,
        };
        await query(`INSERT INTO vantara_deleted_works (series_ref, series_title, deleted_by, reason, snapshot)
            VALUES ($1, $2, $3, $4, $5)
       ...
       VALUES ($1, 'work.soft_delete', $2, $3)`, [session.userId, input.seriesRef, JSON.stringify(snapshot)]);
    ...
    app.get('/v1/deleted-works', { preHandler: requireSession(ctx) }, async (_request, reply) => {
        const rows = await query(`SELECT d.series_ref AS "seriesRef", d.series_title AS "seriesTitle",
    ...
    app.post('/v1/deleted-works/:seriesRef/restore', { preHandler: requireSession(ctx) }, async (request, reply) => {
        const { seriesRef } = request.params;
        const session = sessionOf(request);
        const user = await ctx.uchiyomi.me(session.token);
        if (user.role !== 'admin')
            return reply.code(403).send({ error: 'admin_only' });
        const row = await queryOne(`UPDATE vantara_deleted_works SET restored_at = now()
        WHERE series_ref = $1 AND restored_at IS NULL
        RETURNING series_ref`, [seriesRef]);
        if (!row)
            return reply.code(404).send({ error: 'not_found' });
        await query(`INSERT INTO vantara_audit_log (actor_id, action, target)
       VALUES ($1, 'work.restore', $2)`, [session.userId, seriesRef]);
