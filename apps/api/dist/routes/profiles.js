...
export async function profileRoutes(app, ctx) {
    app.get('/v1/profiles/:username', { preHandler: requireSession(ctx) }, async (request, reply) => {
        ...
        const row = await queryOne(`${SELECT_PROFILE} WHERE u.username = $1`, [username]);
        if (!row)
            return reply.code(404).send({ error: 'not_found' });
        return reply.send(present(row));
    });
    app.get('/v1/profiles', { preHandler: requireSession(ctx) }, async (_request, reply) => {
        const rows = await query(`${SELECT_PROFILE} ORDER BY u.first_seen_at`);
    ...
    app.patch('/v1/profiles/me', { preHandler: requireSession(ctx) }, async (request, reply) => {
        ...
        const row = await queryOne(`UPDATE vantara_profiles SET
    ...
    app.put('/v1/profiles/me/adult', { preHandler: requireSession(ctx) }, async (request, reply) => {
        ...
        WHERE uchiyomi_user_id = $1
        RETURNING adult_enabled, adult_confirmed_at`, [session.userId, parsed.data.enabled]);
        await query(`INSERT INTO vantara_audit_log (actor_id, action, target, detail)
       VALUES ($1, 'adult_gate.set', $2, $3)`, [session.userId, session.username, JSON.stringify({ enabled: parsed.data.enabled })]);
        return reply.send({
