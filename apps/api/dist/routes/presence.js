...
import { HEARTBEAT_INTERVAL_MS, creditForBeat, redactForViewers, statusFor, } from '@vantara/domain';
...
export async function presenceRoutes(app, ctx) {
    /**
    ...
    app.post('/v1/presence/beat', {
        ...
        const credited = await transaction(async (client) => {
            await client.query(`INSERT INTO vantara_presence
           ...
           ON CONFLICT (uchiyomi_user_id) DO UPDATE SET
            ...
            if (beat.seriesRef === undefined)
                return 0;
            const { rows } = await client.query(`SELECT id, last_beat_at, active_ms
             FROM vantara_reading_sessions
            WHERE uchiyomi_user_id = $1
              AND series_ref = $2
              AND coalesce(chapter_ref, '') = coalesce($3, '')
              AND ended_at IS NULL
            FOR UPDATE`, [userId, beat.seriesRef, beat.chapterRef ?? null]);
            const open = rows[0];
            if (!open) {
                await client.query(`INSERT INTO vantara_reading_sessions
               (uchiyomi_user_id, series_ref, chapter_ref, interactions, pages_seen)
             VALUES ($1, $2, $3, $4, $5)`, [userId, beat.seriesRef, beat.chapterRef ?? null, beat.interactions, beat.pagesSeen ?? 0]);
            ...
            await client.query(`UPDATE vantara_reading_sessions
              SET active_ms = active_ms + $2,
                  interactions = interactions + $3,
                  pages_seen = GREATEST(pages_seen, $4),
                  last_beat_at = now()
            WHERE id = $1`, [open.id, credit, beat.interactions, beat.pagesSeen ?? 0]);
            return credit;
        });
    ...
    app.post('/v1/presence/leave', { preHandler: requireSession(ctx) }, async (request, reply) => {
        const { userId } = sessionOf(request);
        await query(`UPDATE vantara_reading_sessions SET ended_at = now()
        WHERE uchiyomi_user_id = $1 AND ended_at IS NULL`, [userId]);
        await query(`UPDATE vantara_presence SET status = 'ONLINE', series_ref = NULL, series_title = NULL,
              chapter_ref = NULL, chapter_label = NULL, progress = NULL, updated_at = now()
        WHERE uchiyomi_user_id = $1`, [userId]);
    ...
    app.get('/v1/presence', { preHandler: requireSession(ctx) }, async (_request, reply) => {
        const rows = await query(`SELECT p.uchiyomi_user_id, u.username, p.series_title, p.chapter_label, p.progress,
              p.updated_at,
        ...
        const content = rows.map((row) => {
            ...
            const status = statusFor(ago, { reading: row.reading });
            const full = {
    ...
    app.put('/v1/presence/incognito', { preHandler: requireSession(ctx) }, async (request, reply) => {
        ...
        const row = await queryOne(`UPDATE vantara_user_gates
          SET incognito_until = CASE WHEN $2::text IS NULL
                                     THEN NULL
                                     ELSE now() + $2::interval END
        WHERE uchiyomi_user_id = $1
        RETURNING incognito_until`, [userId, until]);
