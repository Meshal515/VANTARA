...
import { SESSION_COOKIE, requireSession, sessionOf } from "../lib/context.js";
...
export async function authRoutes(app, ctx) {
    ...
    app.get('/v1/auth/accounts', async (_request, reply) => {
        const rows = await query(`SELECT u.username, p.display_name, p.avatar_path
    ...
    app.post('/v1/auth/login', { config: { rateLimit: { max: 10, timeWindow: '5 minutes' } } }, async (request, reply) => {
        ...
        try {
            const session = await ctx.sessions.login(parsed.data.username, parsed.data.password, device);
