...
export async function reportRoutes(app: FastifyInstance, ctx: AppContext): Promise<void> {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (containsSecret({ diagnostics, description })) {
        // لا نكتب شيئًا نشكّ أنه يحمل سرًّا — الوصف اليدوي مثل diagnostics.
        request.log.error('report payload still contained a secret after scrubbing — refusing');
