...
import { maskSpoilers, type ChapterOrder } from '@vantara/domain';
...
async function logActivity(
  actorId: string,
  verb: string,
  seriesRefValue: string | null,
  payload: unknown = {},
): Promise<void> {
  await query(
    `INSERT INTO vantara_activity_events (actor_id, verb, series_ref, payload)
     VALUES ($1, $2, $3, $4)`,
    [actorId, verb, seriesRefValue, JSON.stringify(payload)],
  );
}

export async function socialRoutes(app: FastifyInstance, ctx: AppContext): Promise<void> {
  // ───────────────────────────── التعليقات ─────────────────────────────
  ...
  app.post('/v1/comments', { preHandler: requireSession(ctx) }, async (request, reply) => {
    ...
    if (input.parentId !== undefined) {
      const parent = await queryOne<{ series_ref: string }>(
        `SELECT series_ref FROM vantara_comments WHERE id = $1 AND deleted_at IS NULL`,
        [input.parentId],
      );
  ...
  app.get('/v1/comments/:seriesRef', { preHandler: requireSession(ctx) }, async (request, reply) => {
    const { seriesRef: ref } = request.params as { seriesRef: string };
    const progress = z.coerce
      .number()
      .optional()
      .safeParse((request.query as { progress?: string }).progress);

    const rows = await query<CommentRow>(
      `SELECT c.id, u.username AS author, c.body, c.chapter_ref, c.spoiler_after_ref,
    ...
    const readerProgress: ChapterOrder | undefined =
      progress.success && progress.data !== undefined
        ? { number: progress.data, kind: 'numbered' }
        : undefined;

    const masked = maskSpoilers(
      rows,
      (row) =>
        row.spoiler_after_ref === null
          ? undefined
          : { number: Number(row.spoiler_after_ref), kind: 'numbered' },
      readerProgress,
    );
  ...
  app.delete('/v1/comments/:id', { preHandler: requireSession(ctx) }, async (request, reply) => {
    ...
    const { userId } = sessionOf(request);
    const row = await queryOne<{ id: string }>(
      `UPDATE vantara_comments SET deleted_at = now()
  ...
  app.put('/v1/comments/:id/reactions/:emoji', { preHandler: requireSession(ctx) }, async (request, reply) => {
    ...
    if ([...emoji].length > 4) return reply.code(400).send({ error: 'bad_request' });

    const { userId } = sessionOf(request);
    await query(
      `INSERT INTO vantara_comment_reactions (comment_id, uchiyomi_user_id, emoji)
  ...
  app.delete('/v1/comments/:id/reactions/:emoji', { preHandler: requireSession(ctx) }, async (request, reply) => {
    ...
    const { userId } = sessionOf(request);
    await query(
      `DELETE FROM vantara_comment_reactions
        WHERE comment_id = $1 AND uchiyomi_user_id = $2 AND emoji = $3`,
      [id, userId, emoji],
    );
    return reply.code(204).send();
  });
  ...
  app.post('/v1/recommendations', { preHandler: requireSession(ctx) }, async (request, reply) => {
    ...
    if (input.to !== 'all') {
      const target = await queryOne<{ uchiyomi_user_id: string }>(
        `SELECT uchiyomi_user_id FROM vantara_users WHERE username = $1`,
        [input.to],
      );
      if (!target) return reply.code(404).send({ error: 'recipient_not_found' });
    ...
    const row = await queryOne<{ id: string }>(
      `INSERT INTO vantara_recommendations (from_id, to_id, series_ref, series_title, message)
       VALUES ($1, $2, $3, $4, $5) RETURNING id`,
      [session.userId, toId, input.seriesRef, input.seriesTitle ?? null, input.message ?? null],
    );
  ...
  app.get('/v1/recommendations/inbox', { preHandler: requireSession(ctx) }, async (request, reply) => {
    ...
    const rows = await query(
      `SELECT r.id, u.username AS "from", r.series_ref AS "seriesRef",
  ...
  app.put('/v1/recommendations/:id/state', { preHandler: requireSession(ctx) }, async (request, reply) => {
    ...
    const { userId } = sessionOf(request);
    const row = await queryOne<{ id: string }>(
      `UPDATE vantara_recommendations SET state = $3
        WHERE id = $1 AND (to_id = $2 OR (to_id IS NULL AND from_id <> $2))
        RETURNING id`,
      [id, userId, parsed.data.state],
    );
    if (!row) return reply.code(404).send({ error: 'not_found' });
    return reply.code(204).send();
  });
  ...
  app.get('/v1/activity', { preHandler: requireSession(ctx) }, async (request, reply) => {
    ...
    const rows = await query(
      `SELECT e.id, u.username AS actor, e.verb, e.series_ref AS "seriesRef",
  ...
  app.get('/v1/read-together/:seriesRef', { preHandler: requireSession(ctx) }, async (request, reply) => {
    const { seriesRef: ref } = request.params as { seriesRef: string };
    const rows = await query(
      `SELECT u.username, p.chapter_label AS "chapterLabel", p.progress,
  ...
  app.get('/v1/stats/me', { preHandler: requireSession(ctx) }, async (request, reply) => {
    ...
    const row = await queryOne<{
      ...
      month_ms: string | null;
      sessions: string;
      avg_ms: string | null;
      longest_ms: string | null;
      series: string;
    }>(
      `SELECT sum(active_ms)::text AS total_ms,
    ...
    const ms = (value: string | null | undefined) => Number(value ?? 0);
    return reply.send({
