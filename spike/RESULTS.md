...
> **ما هذا الملف وما ليس به.** سجلّ قياسٍ من 2026‑09‑16، صالحٌ كدليل على ما
> جُرِّب يومها. وهو **ليس وصفًا للمعمارية الحالية**: بعده أُضيفت المزامنة على
> Cloudflare، ثم أثبت `spike/extension-engine` أن إضافات Keiyoushi تعمل داخل
> الـAPK فخرجت القراءة من مسار Uchiyomi/Suwayomi كلّه. المعمارية العاملة في
> [`../README.md`](../README.md).
...
| 7–8. XianScan + الزمن | ⏳ **لم يُنفّذ هنا** — يحتاج جهاز مشعل (§7) |
| 9. الرسم العربي | ✅ **محسوم بالبكسل** — §7.5 |
...
**الخطوتان 7 و8 تُنفَّذان على جهاز مشعل، ولا تُفوَّضان.** انظر `SPIKE.md`.

---

## 7.5 الرسم العربي — الخطوة 9 ✅ محسومة هنا

الخطوة 9 لا تحتاج XianScan ولا GPU: سؤالها هو **هل يمكن رسم عربي سليم داخل بالون؟**
وهذا قابل للاختبار مستقلًا. فُعل.

`Pillow 12.3` يشحن **raqm 0.10.5** (HarfBuzz + FriBidi + libraqm) داخل الويل:

```
raqm available: True | version: 0.10.5
freetype: 2.14.3
```

رُسمت أربع جمل من الصفحة الحقيقية التي نزّلناها في §4، بمحركين، في بالونات 420×130:

| # | النص | BASIC | RAQM |
|---|---|---|---|
| 1 | `تحذير! تحذير!` | `اريذحت !اريذحت` ❌ | ✅ سليم |
| 2 | `كشف جسم طائر على بعد 2 كم.` | معكوس ❌ | ✅ سليم |
| 3 | `جسم طائر...؟` | معكوس ❌ | ✅ سليم |
| 4 | `من أنت؟ ولماذا تتبعني منذ البارحة؟` | معكوس + التفاف خاطئ ❌ | ✅ سطران سليمان |

الناتج: [`out/arabic-typeset-basic-vs-raqm.png`](./out/arabic-typeset-basic-vs-raqm.png)
المولّد: [`probes/04-arabic-typeset.py`](./probes/04-arabic-typeset.py)

### الفحص البصري الثماني (من `SPIKE.md` §C-4) على عمود RAQM
...
| 3 | الترقيم (`؟` `.` في موضعه) | ✅ |
| 4 | التفاف الأسطر | ✅ |
| 5 | الحجم داخل البالون | ✅ عبر autosize |
| 6 | التمركز | ✅ `anchor="ma"` |
| 7 | التشكيل الرباعي | ✅ |
| 8 | لا نص قديم | — يحتاج inpainting فعليًا (الخطوة 7) |

### ثلاثة أرقام تدخل التنفيذ

```
1. direction="rtl" يرفع KeyError بلا libraqm
   ⇒ الworker يتحقق من features.check("raqm") عند الإقلاع أو يرفض العمل.

2. RAQM أضيق من BASIC بـ20–23%:
   "تحذير! تحذير!"              BASIC 201.0  RAQM 158.6  Δ 42.4
   "كشف جسم طائر على بعد 2 كم."  BASIC 426.0  RAQM 350.8  Δ 75.2
   "من أنت؟ ولماذا تتبعني..."     BASIC 515.0  RAQM 395.2  Δ 119.8
   ⇒ أي معايرة bubble-fitting على مقاسات BASIC خاطئة.

3. لا font fallback في Pillow — NotoNaskhArabic يرسم اللاتيني مربعات فارغة.
   ⇒ البالون المختلط (أسماء، مؤثرات صوتية) يحتاج تقسيم runs + خط لكل run.
```

### الأثر المعماري

```
قبل:  XianScan يترجم ويرسم  →  الرسم العربي مجهول، تقدير أسابيع
بعد:  XianScan/MIT يعطي regions + صورة منظّفة
      نحن نرسم العربي بـPillow/RAQM  →  محلول بمكتبة قياسية
```

**هذا يسقط أكبر مجهول في المشروع.** انظر D-04.
