...
def select_extensions(
    ...
    *,
    ...
    consented_nsfw_packages: set[str] | None = None,
) -> list[dict]:
    """Apply owner policy before any APK bytes are downloaded.

    NSFW is excluded unless the package is named in the separate consent list.
    An optional package allowlist narrows a diagnostic batch without changing
    the pinned Keiyoushi snapshot semantics. Specialized BL/GL remains blocked
    even when accidentally named in the consent list.
    """
    consented_nsfw_packages = consented_nsfw_packages or set()
    selected: list[dict] = []
    for ext in extensions:
        ...
        if content_warning == "NSFW" and package_name not in consented_nsfw_packages:
            continue

        policy_block = blocked_reason(ext, arabic)
        if policy_block is not None and package_name in consented_nsfw_packages:
            continue
...
def main() -> None:
    ...
    raw_nsfw_consent = os.environ.get("VANTARA_NSFW_SOURCE_PACKAGES", "")
    consented_nsfw_packages = {
        p.strip() for p in raw_nsfw_consent.split(",") if p.strip()
    }
    if allowed_packages is None and consented_nsfw_packages:
        raise RuntimeError("NSFW consent requires an explicit VANTARA_SOURCE_PACKAGES allowlist")
    if allowed_packages is not None:
        unlisted_consent = sorted(consented_nsfw_packages - allowed_packages)
        if unlisted_consent:
            raise RuntimeError(
                "NSFW consent contains packages outside the batch allowlist: "
                + ", ".join(unlisted_consent)
            )
    selected = select_extensions(
        extensions,
        allowed_packages=allowed_packages,
        consented_nsfw_packages=consented_nsfw_packages,
    )
    ...
    if allowed_packages is not None:
        ...
        if missing:
            raise RuntimeError(
                "batch allowlist contains missing or policy-ineligible packages: "
                + ", ".join(missing)
            )
