...
interface AnimeCatalogueSource : AnimeSource {
    ...
    suspend fun getPopularAnime(page: Int): AnimesPage {
        throw UnsupportedOperationException("getPopularAnime")
    }
    ...
    suspend fun getSearchAnime(page: Int, query: String, filters: AnimeFilterList): AnimesPage {
        throw UnsupportedOperationException("getSearchAnime")
    }
    ...
    override suspend fun getRelatedAnimeList(
        ...
        if (disableRelatedAnimes) return
        try {
            if (supportsRelatedAnimes && !disableRelatedAnimesBySearch) getRelatedAnimeListByExtension(anime, pushResults)
        } catch (e: Throwable) {
            exceptionHandler(e)
        }
    }
    ...
    suspend fun getRelatedAnimeListByExtension(
        ...
        pushResults(Pair("", fetchRelatedAnimeList(anime)), true)
    }
    ...
    fun String.stripKeywordForRelatedAnimes(): List<String> {
        return split(Regex("[\\s\\-:~_!?.,]+")).filter { it.length > 2 }
    }
