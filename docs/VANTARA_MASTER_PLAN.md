...
**الحالة:** 🧪 **جزئي · `BLOCKED` على الإغلاق** — العقد موثَّق في `docs/VANTARA_TRANSPORT.md`، والمنفَّذ منه لا يعتمد على شكل الـtoken. الإغلاق ينتظر `DONE ✅` لـB2، وهذا حدّ معلن من أول claim لا اعتذار.
...
**ينتظر B2 (معلن):** تحويل JSON إلى Bearer (شكل الـtoken ملكك)، و`SameSite` للكوكي في `routes/auth.ts` — **ملفك، لم ألمسه**. وتقاعد `/v1/img/*` المعتمد على الكوكي يحتاج B6 أيضًا لأنه يملك مسارات الصور الحالية.

**الوضع الآن:** الويب يعمل كما كان، والـAPK يكلّم الـAPI (CORS)، ويحفظ التقدم (credentials)، **ويعرض الصور** (الرابط الموقَّع) — فـ`BE-P0-06` مكتمل. ويبقى نداء JSON على الكوكي حتى يستقر عقد B2.
