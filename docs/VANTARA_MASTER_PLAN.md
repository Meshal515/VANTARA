...
**ينتظر B2 (معلن):** تحويل JSON إلى Bearer (شكل الـtoken ملكك)، وربط الرابط الموقَّع بمسار الصور (من هو المستخدم ومن أين السرّ)، و`SameSite` للكوكي في `routes/auth.ts` — **ملفك، لم ألمسه**. وتقاعد `/v1/img/*` المعتمد على الكوكي يحتاج B6 أيضًا لأنه يملك مسارات الصور الحالية.

**الوضع الآن:** الويب يعمل كما كان، والـAPK يستطيع أن يكلّم الـAPI ويحفظ التقدم، وتبقى **الصور** على الكوكي — البند الوحيد المتبقي من `BE-P0-06`.
