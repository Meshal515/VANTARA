...
| قناع الحروف ومضلّع الفقاعات يمرّان على كل قطع الصفحة (شريط مانهوا: ٦–١٣ قطعة) | القطع التي تمسّ صفوف النص وحدها (من الكاشف، بهامش المناطق للحروف وبعرض الصفحة للفقاعات): كل بكسل يقرؤه التجميع يأخذ القطع نفسها، فالناتج نفسه بتًّا بتًّا |
| المصغّرة تطلب القفل مرة ثانية فتنتظر خلف صفحة أخرى كاملة (~٢٠ ث في تقرير الهاتف) | تُصنع داخل التحليل بالقفل نفسه |
| كل صفحة تنتظر أي صفحة سبقتها إلى القفل؛ صفحة القارئ خلف صفحات الترجمة المقدّمة؛ صفحة ردّت عليها Luna (٤ ث رسم) خلف تحاليل جديدة (٢٠ ث) — تقرير 0.0.33: `render.queue` ١٣٤ ث، وصفحة بلا نص ١١٦ ث لكشف ٠٫٣ ث | صف بترتيب ثابت يُسلَّم فيه الدور مباشرة: الكشف وحده أولًا (صفحة بلا نص تنتهي فيه)، ثم إكمال صفحة القارئ، ثم تحليلها، ثم إكمال المقدّمة، ثم تحليلها؛ ومن وصل أولًا في الرتبة نفسها. `queue` منفصل عن العمل، و«المعالج مشغول ٪» في التقرير |
| LaMa يُحمَّل حين تنتظره الصفحة | يُحمَّل في الخلفية أثناء انتظار Luna لأول صفحة فيها نص، خارج الصف (لا يوقف أحدًا) |
| الترجمة المقدّمة ٣ صفحات معًا | ٤: المعالج يحلّل التالية ما دامت السابقات تنتظر Luna |
...
**الصفحات الطويلة بمقاسها**: التحليل (النماذج والأقنعة) على صفحة أطول ضلعها ≤ 4096، لكن
الملف النهائي بمقاس الصفحة دائمًا: صفحة أطول (شرائط المانهوا ٦٠٠٠–١٥٠٠٠) يُعاد مسحها
بأقنعة مكبّرة ويُرسم عربيّها بالتخطيط نفسه مكبّرًا (خط متجهي حادّ)، وما سواهما بكسلات
الملف. كانت تُحفظ بمقاس التحليل فتُمطّ على الشاشة مبكسلة. والقارئ يعيد ترجمة المحفوظ
الأقصر من أصله (العربي من الخادم بلا Luna).

**نت ضعيف** (بنق ٦٠٠–١٠٠٠ مللي ثانية): الطلب إلى Luna يُرسل أولًا بلا صورة، فصفحة
ترجمتها أنت أو صديق ترجع بلا رفع (`need_image` يطلب الصورة لصفحة جديدة وحدها). المصغّرة
عرضها ≤ 1080 وبكسلاتها ≤ 3.2 مليون بجودة 78 (كانت 1400 بجودة 86). الترجمة المقدّمة لا
تبدأ صفحة جديدة والقارئ يترجم (النت للصفحة أمامك)، وطلب Luna يُقطع بعد ٤ دقائق فيُعاد
بدل أن يعلق. والصفحات المترجمة في مجلد ملفات التطبيق لا الكاش («تحسين الجهاز» يفرغ
الكاش فتعود إنجليزية)، مسقوفة بـ٢٫٥ جيجا يُحذف أقدمها أولًا.

**حروف عند حافة الفقاعة**: مضلّع الفقاعة المكتشف قد يقصر عن الفقاعة الحقيقية (فقاعة
فوق حدّ لوحة داكنة)، فتبقى حروف إنجليزية عند حافتها فاتها قناع الحروف. في الفقاعة
المسطّحة يُؤخذ داخلها الحقيقي من الصورة: ورق بلونها متصل بما حول النص ومحبوس بإطارها،
وكل علامة محاطة بالورق كليًّا على أسطر النص وبحجم حرف تُمسح. ورق يسيل خارج الفقاعة
(إطار مفتوح) يلغي هذه الخطوة كلها. على الصفحات الحقيقية الاثنتي عشرة لم تُضف بكسلًا
واحدًا (`EdgeInkTest` للحالة نفسها).
