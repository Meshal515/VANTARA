...
2. **المرآة مشتقة ومُعلَّمة.** يجوز لمخزن أن يحمل نسخة للمزامنة بين الأجهزة أو
   ...
   المالك للخلف.
3. **الجدول المتقاعد لا يُكتب ولا يُقرأ.** الجداول الاجتماعية القديمة أُسقطت
   فيزيائيًا بـ`packages/db/migrations/0006_drop_retired_social.sql`، والحرس
   يمنع أي مسار SQL من إعادة إحيائها.
...
## 6. حالة الـhandoff بعد Backend Freeze

| البند | الحالة الحالية |
|---|---|
| إسقاط جداول PostgreSQL المتقاعدة فيزيائيًا | ✅ أُنجز في `0006_drop_retired_social.sql` |
| إزالة `vantara_profiles` و`vantara_user_gates` من `sessions.ts` و`auth.ts` | ✅ أُنجز؛ قائمة الاستثناءات في Safety Guard أصبحت فارغة |
| حجب الحارق (spoiler masking) عند العرض | Client/Presentation scope؛ الصف يحمل `spoiler_after` ودالة المجال جاهزة |
| «من وصل أي فصل» (Read Together) | Social/Product presentation فوق `presence` + `progress`، لا يغيّر ملكية البيانات |
| واجهة تفعيل الإخفاء وبوابة محتوى البالغين | F10؛ backend path موجود عبر `settings.patch` و`incognitoUntil` |
| فرض بوابة محتوى البالغين على الاستكشاف | Product/Explore policy؛ لا يعيد فتح Data Ownership |

لا يوجد في Backend Freeze أي handoff مفتوح يسمح بعودة PostgreSQL كمالك Social ثانٍ.
