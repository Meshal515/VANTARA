 ...
 */
export function scrubDiagnostics(input, depth = 0) {
    if (depth > MAX_DEPTH)
        return REDACTED;
...
export function containsSecret(value) {
    ...
    const walk = (node, depth) => {
        ...
        return Object.entries(node).some(([key, child]) => (FORBIDDEN_KEY.test(key) && child !== REDACTED) || walk(child, depth + 1));
    };
    return walk(value, 0);
}
