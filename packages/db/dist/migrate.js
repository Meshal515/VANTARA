...
export async function migrate(connectionString, opts = {}) {
    const log = opts.log ?? (() => { });
    ...
    try {
        ...
        const { rows } = await client.query('SELECT name, checksum FROM vantara_migrations');
        ...
        for (const name of files) {
            ...
            if (previous !== undefined) {
                if (previous !== checksum) {
                    throw new Error(`migration ${name} changed after being applied — ` +
